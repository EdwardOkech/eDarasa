...
$(document).ready(function() {
    ...
    // Notification box
		$(function() {
		  $('.container').on('click', '.notify', function(){
...
function initPopup(){
    var cookie = document.cookie.match('(^|;) ?' + 'popup' + '=([^;]*)(;|$)');
    if (cookie == null || cookie[2] != 'new01'){
        insertPopup();
        initScripts();
    } else {
        return
...
function initScripts(){
    ...
    $.getScript("js/velocity.min.js",function(){
        $.getScript("js/velocity.ui.min.js", function(){
            $.getScript("js/quttons.js", function(){
                    var $footer = $('.footer');
    ...
    if (!$.event.special.inview){
        $.getScript("js/jquery.inview.js")
    }
}

function insertPopup(){

    var textTitle = 'Glad to see you visiting Allec page! Fresh news from Allec authors ;)',
            textNews = 'Long-waited functionality is already here. Starting from version 1.10 Allec template supports RTL layout and allows you create boxed layout pages.',
            textLink01 = 'RTL Preview',
            link01 = 'index-rtl.html',
            textLink02 = 'boxed Preview',
            link02 = 'index2-boxed.html';
    ...
    var $authorContainer = $('<div/>', {
        'class': "popup-news__author"
    }).appendTo($popupContainer);
    
    $('<span class="popup-news__close close"><i class="fa fa-times"></i></span>').appendTo($popupContainer).click(insertCookie);

    $('<img>', {
        'class': "popup-news__author-photo",
        'src': "images/author-photo.jpg",
        'alt': "Author photo"
    }).appendTo($authorContainer);

    $('<h2/>', {
        'class': "popup-news__author-title",
        text: 'Hi, I’m Olia'
    }).appendTo($authorContainer);

    $('<span/>', {
        'class': "popup-news__author-info",
        text: 'author from Congruity Hub'
    }).appendTo($authorContainer);

    $('<h3/>', {
        'class': "popup-news__title",
        text: textTitle
    }).appendTo($popupContainer);

    $('<p/>', {
        'class': "popup-news__text",
        text: textNews
    }).appendTo($popupContainer);

    $('<a/>', {
        'class': "popup-news__link",
        'href': link01,
        text: textLink01
    }).appendTo($popupContainer).click(insertCookie);

    $('<a/>', {
        'class': "popup-news__link popup-news__link-right",
        'href': link02,
        text: textLink02
    }).appendTo($popupContainer).click(insertCookie);
...
function insertCookie(){
    ...
    document.cookie = 'popup=new01;expires=' + expires.toUTCString();
}

function initQutton(){
    var quttonUpload = Qutton.getInstance($('#qutton_upload'));
    quttonUpload.init({
            icon: 'images/icon-popup.svg',
            iconAfterClose: 'images/popup/icon-popup-close.svg',
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    scriptScroll.src = "js/scrollbar.js";
    scriptScroll.defer = 'true';
