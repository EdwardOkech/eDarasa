(function($) {
	...
	*********************************************/
	...
	function Qutton(jQueryDOMElement) {
		...
		this.dialogConfig = {
			...
			backgroundColor : this.$dialog.css('background-color'),
   ...
   if(dialog.isOpen){
	...
	Qutton.prototype.setIconAfterClose = function() {
		this.$container.css('background-image', 'url(' + this.quttonConfig.iconAfterClose + ')');
		this.$container.css('cursor', 'pointer');
		this.$container.css('opacity', '1');
	};
	...
	Qutton.prototype.removeIcon = function() {
		...
		this.$container.removeClass('pulse');
	};
	...
	Qutton.prototype.animateIn = function() {
		...
		var inSequence  = [
          ...
          that.isOpening = false;
			...
			{e : this.$dialog, p : "fadeIn", o : {duration : 100, complete : function() { that.isOpen = true;}}}
		];
	...
	Qutton.prototype.animateOut = function() {
        ...
        that.closing = true;
		var outSequence = [
			{e : this.$dialog, p : "fadeOut", o : {duration : 100,
			complete : function() {
